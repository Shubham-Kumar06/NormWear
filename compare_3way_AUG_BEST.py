"""
3-way comparison on aug-BEST backbone:
  A — frozen encoder + sklearn LogisticRegression  (linear probe)
  B — frozen encoder + MLPHead, same recipe as C   (training-recipe control)
  C — LoRA(rank=4, qkv) + MLPHead                  (LoRA condition)

Δ(C-A) = combined LoRA + training-recipe gain
Δ(C-B) = LoRA-attributable gain  (the rigorous one)
Δ(B-A) = training-recipe-attributable gain
"""
import json
...
SRC = {
    "A_linear_probe":  ROOT / "wesad_baseline_summary_AUG_BEST.json",
    "B_neural_head":   ROOT / "wesad_neuralhead_summary_AUG_BEST.json",
    "C_lora":          ROOT / "wesad_lora_paper_summary_AUG_BEST.json",
}
...
C = load(SRC["C_lora"])
sids = sorted(set(A) & set(B) & set(C), key=int)
...
for sid in sids:
    ...
    rows.append({"subject": sid,
                 ...
                 "delta_C_minus_B": round(c - b, 2),  # the rigorous LoRA-only gain
...
try:
    ...
    p_str = f"(scipy not available: {e})"

# Cohen's d for paired diffs
diffs = [r["delta_C_minus_B"] for r in rows]
...
out = {
    ...
    "conditions": {
        ...
        "C": "LoRA(rank=4 alpha=8 qkv) + MLPHead(256) (Side B of original analysis)",
    },
    "per_subject": rows,
...
out_path = ROOT / "wesad_3way_AUG_BEST.json"
...
for r in rows:
    ...
    print(f"S{r['subject']:<6}{r['A_lin']:>12.2f}{r['B_head']:>10.2f}{r['C_lora']:>10.2f}"
...
print(f"")
print(f"  Δ(C−A) = {mC-mA:+.2f}   <- combined gain (LoRA + recipe vs linear probe)")
print(f"  Δ(C−B) = {mC-mB:+.2f}   <- LoRA-attributable gain (rigorous control)")
print(f"  Δ(B−A) = {mB-mA:+.2f}   <- training-recipe-attributable gain")

print(f"\n=== Paired stats (C vs B, n=15) ===")
print(f"  Wins for LoRA  : {wins}/15")
print(f"  Losses for LoRA: {losses}/15")
print(f"  Ties           : {ties}/15")
