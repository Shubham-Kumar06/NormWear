"""
neuralhead_wesad_run.py  —  Side C control for the LoRA ablation.

Identical to lora_wesad_run.py EXCEPT no LoRA injection:
...
splits) are kept byte-identical so the only variable vs Side B (LoRA) is
the presence/absence of LoRA layers in qkv.

Run from /home/ug24/FoundationalModel/:
    python3 -m NormWear.neuralhead_wesad_run
"""
...
SAVE_PATH   = "/home/ug24/FoundationalModel/NormWear/data/results/lora_results/wesad_neuralhead_summary_AUG_BEST.json"
...
# ── Same training recipe as lora_wesad_run.py ────────────────────────────────
EPOCHS        = 30
WARMUP_EPOCHS = 3
LR            = 3e-4
WEIGHT_DECAY  = 0.1
...
TRIAL_RX = re.compile(r"^(\d+)_(\d+)_(\d+)_(\d+)\.pkl$")


class MLPHead(nn.Module):
    def __init__(self, embed_dim: int = 768, hidden: int = 256, dropout: float = 0.3):
        super().__init__()
...
class NormWearFrozenHead(nn.Module):
    """Frozen encoder + trainable MLP head — Side C of the ablation."""
    def __init__(self, base_encoder):
        super().__init__()
        self.encoder = base_encoder
        # Freeze encoder — NO LoRA injection (this is the difference vs Side B).
        for p in self.encoder.parameters():
            p.requires_grad_(False)
        self.head = MLPHead(embed_dim=768, hidden=256, dropout=0.3)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, nvar, C, L, F = x.shape
        x_in = x.view(B * nvar, C, L, F)
        # Encoder runs in eval mode (no dropout) since it's frozen.
        self.encoder.eval()
        with torch.no_grad():
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  NormWear + Frozen-Encoder + MLPHead  (Side C — no LoRA control)")
    print(f"  epochs={EPOCHS}  recipe matches lora_wesad_run.py")
    print(f"  device={device}")
    print(f"{'='*65}\n")

    encoder_template = load_encoder(CKPT_PATH)
    subject_splits   = _resolve_splits()
    subjects         = sorted(subject_splits.keys(), key=lambda x: int(x))
    ...
    for sid in subjects:
        ...
        if len(set(test_lbls)) < 2:
            print(f"[S{sid}] SKIP — single-class test set"); continue

        print(f"\n{'='*65}")
        print(f"[S{sid}] train_windows={len(train_files)}  test_windows={len(test_files)}")
        try:
            auc = train_one_subject(sid, train_files, test_files, encoder_template, device)
        except Exception as e:
            import traceback; traceback.print_exc()
            print(f"[S{sid}] ERROR: {e}"); continue
        print(f"[S{sid}] *** test_AUC = {auc:.2f}% ***")
    ...
    print(f"\n{'='*65}")
    print(f"  WESAD Side C (Frozen + MLPHead, no LoRA) Results")
    print(f"{'='*65}")
    for r in per_subject:
        print(f"  S{r['subject_id']:>3}  test_AUC = {r['auc']:.2f}%")
    print(f"  {'─'*30}")
