...
baseline_amigos.py
==================
Method A for AMIGOS: NormWear frozen encoder + logistic-regression linear probe.

Uses the SAME per-subject trial-level 80/20 splits as lora_amigos_run.py
(loaded from per_subject_splits.json) so results are directly comparable.

Run from /home/ug24/FoundationalModel/:
    python3 -m NormWear.baseline_amigos
"""
...
SAVE_PATH  = "/home/ug24/FoundationalModel/NormWear/data/results/lora_results/amigos_baseline_summary_AUG_BEST.json"
DS_NAME    = "amigos"
PAD_NVAR   = 4
MAX_L      = 390
BATCH_SIZE = 32
...
def main():
    ...
    print(f"[Baseline-AMIGOS] device={device}")

    with open(SPLIT_JSON) as fp:
        subject_splits = json.load(fp)
    print(f"[Baseline-AMIGOS] Loaded splits for {len(subject_splits)} subjects")

    encoder = load_encoder(CKPT_PATH, device)

    per_subject_results = []

    for sid in sorted(subject_splits.keys()):
        ...
        raw    = roc_auc_score(yte, clf.predict_proba(scaler.transform(Xte))[:, 1]) * 100
        auc    = max(raw, 100.0 - raw)  # direction-agnostic: matches lora_amigos_run.py

        print(f"[{sid}] train={n_train:5d}  test={len(test_files):5d}  AUC={auc:.2f}%")
    ...
    print(f"\n[Baseline-AMIGOS] Mean AUC = {summary['mean_auc']:.2f} "
