...
baseline_amigos.py
==================
Per-subject baseline on AMIGOS using the FROZEN NormWear encoder
...
def main():
    ...
    for sid in subjects:
        ...
        for start in range(0, n_total, BATCH_SIZE):
            ...
            batch = torch.stack([prep_input(os.path.join(DATA_DIR, f))
                                 for f in batch_files])   # [B, 4, 3, MAX_L, 65]
        ...
        # Standardize then logistic regression (linear probe)
    ...
    print(f"[Baseline] AMIGOS Mean AUC = {summary['mean_auc']:.2f} "
