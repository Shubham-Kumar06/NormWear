"""
lora_wesad_run.py  —  Paper-exact LoRA on WESAD
================================================
Mirrors lora_amigos_run.py / lora_dreamer_run.py, adapted for WESAD:
  • 10 physiological channels (BVP, EDA, TEMP, ACC x3, chest sensors)
  • Binary stress: class 1 = stress/affective, class 0 = baseline/amusement
  • 15 subjects (2-17, no 12), ~75 trials each
  • Filename format: {subject}_{condition}_{segment}_{window}
...
Run from /home/ug24/FoundationalModel/:
    python3 -m NormWear.lora_wesad_run
"""
...
SAVE_PATH   = "/home/ug24/FoundationalModel/NormWear/data/results/lora_results/wesad_lora_paper_summary.json"
...
SEED     = 42
# filename: {subject}_{condition}_{segment}_{window}
TRIAL_RX = re.compile(r'^(\d+)_(\d+)_(\d+)_\d+$')

# Load pre-computed per-subject splits (generated by prepare_wesad_per_subject_splits.py)
...
def gather_subject_trials():
    """
    Filename: {subject}_{condition}_{segment}_{window}
    Trial key: condition * 1000 + segment  (unique integer per trial)
    """
...
def _resolve_splits() -> dict:
    ...
    result = {}
    for sid in sorted(by_subj_trial.keys(), key=lambda x: int(x)):
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  NormWear + LoRA  [Paper-Exact: WESAD]")
    ...
    for sid in subjects:
        ...
        print(f"[S{sid}] *** test_AUC = {test_auc:.2f}% ***")
        per_subject.append({
            "subject_id":  sid,
            "auc":         float(test_auc),
    ...
    summary = {
        ...
        "mean_auc":    float(np.mean(test_aucs)) if test_aucs else float("nan"),
        "std_auc":     float(np.std(test_aucs))  if test_aucs else float("nan"),
    ...
    print(f"\n{'='*65}")
    print(f"  WESAD LoRA (Paper-Exact) Results")
    print(f"{'='*65}")
