...
class AugmentedPretrainDataset(Dataset):
    """Originals + on-the-fly mixup augmentation (paper Algorithm 1), held in RAM.

    The dataset behaves as a fixed (1 + aug_factor)x corpus: indices [0, n) are the
    originals; indices [n, n*(1+aug_factor)) are augmented samples generated by a
    deterministic chunk-swap between two same-channel-count series. The augmentation
    for a given index is seeded by that index, so the corpus is identical every epoch
    (equivalent to a pre-stored augmented set, but using zero extra disk).
    """
    AUG_SEED = 42

    def __init__(self, data_dir, dataset_names, aug_factor=9, is_test=0):
        from collections import defaultdict
        self.aug_factor = aug_factor
        self.tss = []                       # list[np.ndarray float16], shape [nvar, L]
        self.nvar_groups = defaultdict(list)
        ...
        for fp in tqdm(fnames, desc="Loading originals into RAM"):
            with open(fp, 'rb') as f:
                d = pickle.load(f)
            arr = d['tss']
            arr = arr if isinstance(arr, np.ndarray) else arr.numpy()
            arr = np.ascontiguousarray(arr.astype(np.float16))
            self.nvar_groups[arr.shape[0]].append(len(self.tss))
            self.tss.append(arr)

        self.n_orig = len(self.tss)
        print(f"AugmentedPretrainDataset: {self.n_orig} originals, "
              f"virtual size {len(self)} ({1 + aug_factor}x), "
              f"nvar groups { {k: len(v) for k, v in self.nvar_groups.items()} }")

    def __len__(self):
        return self.n_orig * (1 + self.aug_factor)

    def _augmented_tss(self, idx):
        import random
        k = idx - self.n_orig
        base_idx = k // self.aug_factor
        rng = random.Random(self.AUG_SEED + idx)
        x1 = self.tss[base_idx]
        nvar, L1 = x1.shape
        partner = rng.choice(self.nvar_groups[nvar])
        x2 = self.tss[partner]
        L2 = x2.shape[1]
        Lc = min(L1, L2)
        lam = rng.randint(1, Lc)
        s1 = rng.randint(0, L1 - lam)
        s2 = rng.randint(0, L2 - lam)
        aug = x1.copy()
    ...
    def __getitem__(self, idx):
        if idx < self.n_orig:
            tss_raw = self.tss[idx]
        else:
            tss_raw = self._augmented_tss(idx)

        target = torch.from_numpy(np.ascontiguousarray(tss_raw)).float()
        mean = target.mean(dim=1, keepdim=True)
        std = target.std(dim=1, keepdim=True) + 1e-8
        target = (target - mean) / std
        x = cwt_wrap(target)
