"""
lora_amigos_run.py  —  Paper-exact LoRA implementation (Seregina et al., lora.md)
===================================================================================
Follows the lora.md paper exactly:
  • LoRA injected into BOTH attention (qkv, proj) AND FFN (fc1, fc2)
  • rank=32, alpha=64 (2×rank)   — paper Table VI best-balance point
  • 50 epochs                    — paper Section V-A
  • Head: Linear→BatchNorm→ReLU→Dropout→Linear  (paper Section IV-A-e)
  • 80/20 TRIAL-level split (no window-level leakage)
  • Metric: AUC ROC (binary arousal on AMIGOS)

Run from /home/ug24/FoundationalModel/:
    python3 -m NormWear.lora_amigos_run
"""
...
EPOCHS        = 50         # paper: 50 epochs (Section V-A)
WARMUP_EPOCHS = 5          # ~10% of epochs
LR            = 3e-4
WEIGHT_DECAY  = 1e-2
LORA_RANK     = 32         # paper Table VI: best accuracy-efficiency balance
LORA_ALPHA    = 64.0       # = 2 × rank  (standard LoRA convention)
LORA_DROPOUT  = 0.1        # paper uses standard 0.1

# paper: LoRA in attention AND FFN (Section IV-B)
...
# ── Paper-exact MLP head ──────────────────────────────────────────────────────
class MLPHead(nn.Module):
    """
    Paper (Section IV-A-e): 'small MLP with BatchNorm and Dropout,
    projecting encoder embedding dimension to K classes.'
    Binary → 1 output logit (BCE).
    """
    def __init__(self, embed_dim: int = 768, hidden: int = 256, dropout: float = 0.1):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden, 1),   # binary: single logit + BCE
        )
        self._init_weights()
...
# ── LoRA injection (paper Section IV-B: attention + FFN) ─────────────────────
def inject_lora_paper(model: nn.Module, rank: int, alpha: float,
                      dropout: float, target_modules: tuple) -> list:
    """Freeze all, then inject LoRALinear into target attribute names."""
    ...
    print(f"[LoRA-Paper] Injected into {replaced} Linear layers "
          f"(rank={rank}, alpha={alpha}, targets={target_modules})")
    print(f"[LoRA-Paper] Total params  : {total:,}")
    print(f"[LoRA-Paper] LoRA trainable: {lora:,}  ({100*lora/max(total,1):.2f}%)")
    return lora_params
...
def train_one_subject(sid, train_files, test_files, encoder_template, device):
    ...
    for ep in range(EPOCHS):
        ...
        if len(np.unique(y_arr)) < 2:
            ...
            auc = max(raw, 100.0 - raw)   # absolute deviation from chance
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  NormWear + LoRA  [Paper-Exact: Seregina et al., lora.md]")
    ...
    print(f"\n{'='*65}")
    print(f"  AMIGOS LoRA (Paper-Exact) Results")
    print(f"{'='*65}")
