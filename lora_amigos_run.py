...
# Load pre-computed per-subject splits (generated by prepare_amigos_per_subject_splits.py)
_SUBJECT_SPLITS: dict = {}
if os.path.isfile(SPLIT_JSON):
    with open(SPLIT_JSON) as _fp:
        _SUBJECT_SPLITS = json.load(_fp)
    print(f"[Splits] Loaded per-subject splits from {SPLIT_JSON} "
          f"({len(_SUBJECT_SPLITS)} subjects)")
else:
    print(f"[Splits] WARNING: {SPLIT_JSON} not found — will compute splits dynamically")
...
def train_one_subject(sid, train_files, test_files, encoder_template, device):
    ...
    if len(np.unique(y_arr)) < 2:
        ...
        test_auc = max(raw, 100.0 - raw)  # direction-agnostic: AMIGOS labels are not consistently oriented
...
# ── Main ──────────────────────────────────────────────────────────────────────
def _resolve_splits() -> dict:
    """
    Returns {sid: {"train": [fname, ...], "test": [fname, ...]}} for all subjects.
    Uses pre-computed per_subject_splits.json when available; falls back to
    dynamic stratified trial-level 80/20 split.
    """
    if _SUBJECT_SPLITS:
        return _SUBJECT_SPLITS

    print("[Splits] Computing dynamic per-subject trial splits ...")
    by_subj_trial, trial_label = gather_subject_trials()
    rng = np.random.default_rng(SEED)
    result = {}
    for sid in sorted(by_subj_trial.keys()):
        ...
        result[sid] = {
            "train": [f for t in train_t for f in sorted(trials_dict[t])],
            "test":  [f for t in test_t  for f in sorted(trials_dict[t])],
        }
    return result
...
def main():
    ...
    subjects         = sorted(subject_splits.keys())
